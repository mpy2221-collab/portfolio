...
const UserPickList = (props) => {
  ...
  // 유저픽 영화 목록 조회
  useEffect(() => {
    let url = backServer + "/board/user-pick/list/" + reqPage;

    // 검색 모드일 때
    if (searchMode && searchParams) {
      url = backServer + "/board/user-pick/search";
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.message === "success") {
          ...
          setUserPickList(res.data.data.userPickList || []);
        }
      })
      .catch((err) => {
  ...
  const goToMovieDetail = (movieId) => {
    navigate("/board/user-pick/view/" + movieId);
  };
  ...
  return (
    <div className="userpick-list-wrap">
      {/* 검색 영역 */}
      <div className="userpick-search-section">
        <div className="userpick-search-container">
      ...
      {/* 영화 리스트 영역 */}
      <div className="userpick-movies-section">
        <div className="userpick-movies-grid">
          {userPickList.length > 0 ? (
            userPickList.map((movie, index) => (
              <div
                key={movie.userpickMovieNo || index}
                className="movie-card"
                onClick={() => goToMovieDetail(movie.userpickMovieTmdbMovieId)}
              >
                <div className="movie-poster-wrapper">
                  <img
                    src={getPosterUrl(movie.userpickMoviePosterPath)}
                    alt={movie.userpickMovieTitle}
                ...
                <div className="movie-info">
                  ...
                  {/* 유저 평가 정보 */}
                  {movie.userpickMovieRating !== undefined &&
                    movie.userpickMovieReviewCount > 0 && (
                      <div className="movie-user-rating-info">
                        <span className="user-rating-label">유저 평가:</span>
                        <span className="user-rating-value">
                          {formatRating(movie.userpickMovieRating)} / 10
                        </span>
                        {movie.userpickMovieReviewCount !== undefined && (
                          <span className="user-rating-count">
                            ({movie.userpickMovieReviewCount}명 평가)
                          </span>
                        )}
                      </div>
                    )}
