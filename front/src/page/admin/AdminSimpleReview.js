...
const AdminSimpleReview = () => {
  ...
  useEffect(() => {
    axios
      .get(backServer + "/admin/simple-review/statistics/all")
      .then((res) => {
  ...
  // 심플 리뷰 목록 조회
  useEffect(() => {
    let url = backServer + "/admin/simple-review/list/" + reqPage;

    // 검색 모드일 때
    if (searchMode && searchParams) {
      url = backServer + "/admin/simple-review/search";
  ...
  const handleUserSearch = () => {
    ...
    axios
      .get(backServer + "/admin/simple-review/statistics/user", {
  ...
  // 심플 리뷰 삭제
  const handleDeleteReview = (reviewNo, movieTitle) => {
    Swal.fire({
      title: "심플 리뷰 삭제",
      text: `정말로 "${movieTitle}" 리뷰를 삭제하시겠습니까?`,
      icon: "warning",
      ...
      if (result.isConfirmed) {
        axios
          .delete(backServer + "/admin/simple-review/" + reviewNo)
          .then((res) => {
            if (res.data.message === "success") {
              Swal.fire({
                title: "삭제 완료",
  ...
  return (
    <div className="admin-simple-review-wrap">
      {/* 헤더 */}
      <div className="admin-simple-review-header">
        <h2 className="admin-simple-review-title">심플 리뷰 관리</h2>
        <p className="admin-simple-review-subtitle">
          전체 심플 리뷰를 확인하고 관리할 수 있습니다.
        </p>
      </div>

      {/* 통계 영역 1 (전체 유저 통계) */}
      <div className="admin-simple-review-statistics-all">
      ...
      {/* 통계 영역 2 (특정 유저 통계) */}
      <div className="admin-simple-review-statistics-user" ref={userStatisticsRef}>
      ...
      {/* 검색 영역 */}
      <div className="admin-simple-review-search">
        <div className="admin-simple-review-search-container">
      ...
      {/* 심플 리뷰 리스트 */}
      <div className="admin-simple-review-list">
        {reviews.length === 0 ? (
                ...
                <div className="review-info">
                  ...
                  <div className="review-author">
                    ...
                    <span
                      ...
                      onClick={() =>
                        handleAuthorClick(review.simpleReviewMemberId)
                      }
                    >
                      {review.simpleReviewMemberId || "-"}
                    </span>
                  </div>
                  {/* 평점 */}
      ...
      {/* 페이지네이션 */}
      {pageInfo && (
        <div className="admin-simple-review-pagination">
          <Pagination
            pageInfo={pageInfo}
            reqPage={reqPage}
...
export default AdminSimpleReview;
