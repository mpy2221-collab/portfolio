...
const MemberBoardReview = () => {
  ...
  useEffect(() => {
    axios
      .get(backServer + "/member/board-review/statistics")
      .then((res) => {
  ...
  // 게시글 리뷰 목록 조회
  useEffect(() => {
    let url = backServer + "/member/board-review/list/" + reqPage;

    // 검색 모드일 때
    if (searchMode && searchParams) {
      url = backServer + "/member/board-review/search";
    ...
    axios
      ...
      .then((res) => {
        if (res.data.message === "success") {
          console.log("게시글 리뷰 목록:", res.data.data);
  ...
  return (
    <div className="member-board-review-wrap">
      {/* 헤더 */}
      <div className="member-board-review-header">
        <h2 className="member-board-review-title">게시글 리뷰</h2>
        <p className="member-board-review-subtitle">
          내가 작성한 게시글 리뷰를 확인하고 관리할 수 있습니다.
        </p>
      </div>

      {/* 통계 영역 */}
      <div className="member-board-review-statistics">
        <StatisticsBox
          statistics={[
            {
              label: "작성한 게시글 리뷰",
      ...
      {/* 검색 영역 */}
      <div className="member-board-review-search">
        <div className="member-board-review-search-container">
      ...
      {/* 게시글 리뷰 리스트 */}
      <div className="member-board-review-list">
        {reviews.length === 0 ? (
                ...
                <div className="review-info">
                  ...
                  <div className="review-content">
                    {review.boardReviewContent
                      ? review.boardReviewContent.length > 100
                        ? review.boardReviewContent
                            .replace(/<[^>]*>/g, "")
                            .substring(0, 100) + "..."
                  ...
                  <div className="review-date">
                    작성일: {formatDate(review.boardReviewDate)}
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* 페이지네이션 */}
      {pageInfo && (
        <div className="member-board-review-pagination">
          <Pagination
            pageInfo={pageInfo}
            reqPage={reqPage}
...
export default MemberBoardReview;
