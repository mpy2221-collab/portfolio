...
const FindId = () => {
  ...
  // 아이디 찾기 요청
  const findId = () => {
    if (checkEmailMsg === "" && memberEmail !== "" && isEmailAuthVerified) {
      axios
        .post(backServer + "/member/find-id", { memberEmail: memberEmail })
        .then((res) => {
          if (res.data.message === "success") {
            Swal.fire({
              title: "아이디 발송 완료",
              text: "등록된 이메일로 아이디를 전송했습니다.",
              icon: "success",
              confirmButtonText: "확인",
              confirmButtonColor: "#1a1a1a",
            }).then(() => {
    ...
    <div className="find-id-wrap">
      <div className="find-id-container">
        <div className="find-id-header">
          ...
          <p className="find-id-subtitle">
            등록된 이메일로 아이디를 찾을 수 있습니다
          </p>
        </div>

        <div className="find-id-content">
          <div className="find-id-form-section">
            <h2 className="section-title">이메일 인증</h2>
        ...
        <div className="find-id-btn-section">
          <Button
            text="아이디 찾기"
            type="primary"
            onClick={findId}
            className="btn-full btn-large"
            disabled={!isEmailAuthVerified}
          />
        </div>
      </div>
    </div>
